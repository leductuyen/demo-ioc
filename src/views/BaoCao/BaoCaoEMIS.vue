<template>
    <div>
        <CustomBreadcrumb :title="'BÁO CÁO'" :content="'BÁO CÁO EMIS'" />
        ...
        <div class="layout-table">
            <div>
                <nav>
                    <div class="nav nav-tabs">
                        <a
                            v-for="(tab, index) in dataBaoCaoEMIS"
                            :key="index"
                            :class="{
                                'nav-item': true,
                                'nav-link': true,
            ...
            <div class="report-content">
                <table class="table table-bordered table-hover centered-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-thead-span">
                                ...
                                <span
                                    v-if="
                                        dataBaoCaoEMIS[activeTab] &&
                                        dataBaoCaoEMIS[activeTab].category
                                    "
                                >
                                    ({{ dataBaoCaoEMIS[activeTab].category }})
                                </span>
                    ...
                    <tbody>
                        <tr
                            v-for="(report, index) in dataBaoCaoEMIS[activeTab]
                                ?.children"
                            :key="index"
                        >
        ...
        async handleSearch() {
            try {
                ...
                const response = await sendRequest(
                    Api.baoCao.baoCaoEMIS.danhSachBaoCao,
                    null,
                    request_Header,
                    request_Params
                )
                if (response) {
                    ...
                    setTimeout(() => {
                        loading.close()
                        this.$message({
                            message: 'Tìm kiếm danh sách thành công',
                            type: 'success'
                        })
                    }, 1000)
                }
                this.dataBaoCaoEMIS = response
            } catch (error) {
                console.log(error)
            }
        },
